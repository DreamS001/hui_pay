...
export default {
	...
	methods: {
		// 请求全部数据
		request() {
			logQuery(this.pageNo, this.pageSize, this.count, this.operator_id, this.type, this.addtime).then(res => {
				this.list = eval(res.list)
				this.total = res.total
			});
		},
		getData(){
			logQuery(this.pageNo, this.maxPageSize, this.count, this.operator_id, this.type, this.addtime).then(res => {

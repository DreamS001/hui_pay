      ...
      <div class="nav-a">
		...
		<div class='create_time'>
          ...
          <div class="operation">
            ...
            <span class="time" style="background:#fff;border: 1px solid #DCDFE6;color: #606266;" @click="download">清空</span>
          </div>
        </div>
      </div>
      ...
      if (this.startPlaceHoler=='' || this.endPlaceHoler=='') {
		...
		getData(){
			logQuery(this.pageNumber, this.maxPageSize, this.count, this.operator_id, this.type, this.allTime).then(res => {
				this.allList = eval(res.data.list);
				let datas = JSON.parse(JSON.stringify(this.allList));
				// 转换时间戳
				datas.forEach((v, i) => {
					datas[i].addtime = moment((datas[i].addtime)*1000).format("YYYY-MM-DD hh:mm:ss");
				});
				require.ensure([], () => {

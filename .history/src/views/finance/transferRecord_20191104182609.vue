    ...
    <div class="papel-box">
      ...
      <div style="width:100%!important;" class="table-box">
        <!-- <el-table :data="list" style="width: 100%!important" stripe :header-cell-class-name="handlemyclass">
          ...
          <el-table-column prop="operator_id" label="操作人id" v-model="operator_id"></el-table-column>
        ...
        <el-table :data="list" style="width: 100%!important" stripe :header-cell-class-name="handlemyclass">
          <el-table-column prop="count" :label="meta.titles[0]" v-model="count"></el-table-column>
          <el-table-column prop="type" :label="meta.titles[1]">
            <template slot-scope="scope">{{scope.row.type|formatType}}</template>
          </el-table-column>
          <el-table-column prop="addtime" :label="meta.titles[2]" v-model="addtime"></el-table-column>
          <el-table-column prop="operatorId" :label="meta.titles[3]" v-model="operatorId"></el-table-column>
          <el-table-column prop="webIp" :label="meta.titles[4]"></el-table-column>
          <el-table-column prop="loc_info" label="操作">
...
export default {
	data() {
		return {
			...
			addtime:"",//创建时间
			count:"",//操作内容
			addtime:"2019-10-28,2019-10-29",//操作时间
			operator_id:"",//操作人id
			web_ip:"",//访问ip
	...
	methods: {
        ...
        // this.allList = eval(res.data.list);
				// let date = JSON.parse(JSON.stringify(this.data.allList));
				// // 转换时间戳
				// date.forEach((v, i) => {
        ...
        // res.data.list.forEach((v, i) => {
				// 	res.data.list[i].addtime = moment(res.data.list[i].addtime).format("YYYY-MM-DD");
        // });
        this.list = eval(res.data.list)
				this.total = res.data.pager.
			});
		},
		getData(){

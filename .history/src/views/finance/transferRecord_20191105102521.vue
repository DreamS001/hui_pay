...
export default {
	...
	methods: {
      ...
      if (this.startPlaceHoler=='' || this.endPlaceHoler=='') {
        ...
        this.topaddtime=this.startDate+','+this.endDate
      }
			logQuery(this.pageNumber, this.pageSize, this.count, this.operator_id, this.type, this.topaddtime).then(res => {
		...
		getData(){
			logQuery(this.pageNumber, this.maxPageSize, this.count, this.operator_id, this.type, this.topaddtime).then(res => {
				this.allList = eval(res.data.list);

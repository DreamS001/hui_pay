  ...
  <div class="wscn-http404-container">
    ...
    <div style="width:100%!important;margin:20px">
      ...
      >
        ...
        <el-table-column style="color:red" prop="sta_profit" label="静态钱包收益（美金）" min-width="160"></el-table-column>
        <el-table-column style="color:red" prop="charge_price" label="充值金额（美金）" min-width="150"></el-table-column>
        <el-table-column style="color:red" prop="device_expend" label="购买机器人支出（美金）" min-width="170"></el-table-column>
        <el-table-column style="color:red" prop="cash_withdraw" label="提现支出（美金）" min-width="170"></el-table-column>
    ...
    <div v-if="FC" class="fuCeng">
      <div>
        ...
        <div>
          ...
          >
            <el-table-column prop="date" label="日期" min-width="100"></el-table-column>
            <el-table-column prop="act_profit" label="动态钱包收益（美金）"  min-width="160"></el-table-column>
            <el-table-column prop="sta_profit" label="静态钱包收益（美金）"  min-width="160"></el-table-column>
            <el-table-column prop="charge_price" label="充值金额（美金）"  min-width="150"></el-table-column>
            <el-table-column prop="device_expend" label="购买机器人支出（美金）"  min-width="170"></el-table-column>
            <el-table-column prop="cash_withdraw" label="提现支出（美金）" min-width="170"></el-table-column>
          </el-table>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      if (this.clickQueryDate == false) {
        ...
        if (this.selectTime == true) {
          ...
          cashquery(this.pageNo, this.pageSize, "", "").then(res => {
            this.list = eval(res.list);
    ...
    queryDate() {
      ...
      if (this.selectTime == false) {
        this.$message.warning("您可以选择时间段查询");
      }
      ...
      if (this.selectTime == true) {
        ...
        cashquery(this.pageNo, this.pageSize1, "", "").then(res => {
          this.allList = eval(res.list);
          this.total = res.total;
        });
      }
    ...
    // date.forEach((v, i) => {
    //   // date[i].enabled = date[i].enabled === 1 ? "是" : "否";
        ...
        cashquery(this.pageNo, this.pageSize1, this.time, this.times).then(
          res => {
            ...
            require.ensure([], () => {
              ...
              } = require("@/utils/Export2Excel.js"); //引入文件
              const tHeader = [
                "日期",
                "动态钱包收益(美金)",
                "静态钱包收益(美金)",

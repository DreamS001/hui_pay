    ...
    <div class="papel-box">
      <div class="nav-a">
		...
		<div class='create_time'>
          ...
          <el-date-picker @change="timeChange" v-model="addtime" type="daterange" size="mini" range-separator="至" :start-placeholder="startPlaceHoler" :end-placeholder="endPlaceHoler" format='yyyy-MM-dd' value-format='yyyy-MM-dd'></el-date-picker>
        </div>
          <div class="operation">
...
export default {
	data() {
		return {
			...
			allList:[],
			count:"",//操作内容
			countValue:"",//操作内容保存值，防止因叉号变成undefined
			operator_id:"",//操作人ID
			operator_idValue:"",//操作人ID保存值，防止因叉号变成undefined
			type:"",//用户类型
			typeValue:"",//用户类型保存值，防止因叉号变成undefined
			addtime:"",//创建时间
			addtimeValue:"",//创建时间保存值，防止因叉号变成undefined
			count:"",//操作内容
			addtime:"",//操作时间
			nickname:"",//操作人
			web_ip:"",//访问ip
	...
	methods: {
		// 请求全部数据
		request() {
			logQuery(this.pageNumber, this.pageSize, this.count, this.operator_id, this.type, this.addtimeValue).then(res => {
		...
		getData(){
			logQuery(this.pageNumber, this.maxPageSize, this.count, this.operator_id, this.type, this.addtimeValue).then(res => {
				this.allList = eval(res.data.list);
		...
		timeChange() {
			if (this.addtime == undefined) {
				this.addtimeValue=''
			}else{
				this.addtimeValue=this.addtime
			}
		},
		// 导出
		download() {
